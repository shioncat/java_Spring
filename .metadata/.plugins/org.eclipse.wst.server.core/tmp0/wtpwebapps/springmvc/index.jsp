<%@ page import="java.util.Date"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Index</title>
</head>
<body>
	Index <%=new Date() %>
	<ol>
		<li>
			<a href="./mvc/case01/hello/welcome">./mvc/case01/hello/welcome</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/sayhi?name=vincent&age=18">./mvc/case01/hello/sayhi?name=vincent&age=18</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/bmi?h=170&w=60">./mvc/case01/hello/bmi?h=170&w=60</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/exam/45">./mvc/case01/hello/exam/45</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/calc/add?x=30&y=20">./mvc/case01/hello/calc/add?x=30&y=20</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/path/namejo/java8">./mvc/case01/hello/path/namejo/java8</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/age?age=18&age=19&age=20">./mvc/case01/hello/age?age=18&age=19&age=20</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/javaexam?score=80&score=100&score=50">./mvc/case01/hello/javaexam?score=80&score=100&score=50</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/user?name=John&age=18">./mvc/case01/hello/user?name=John&age=18</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/person?name=John&score=100&age=18&pass=true">./mvc/case01/hello/person?name=John&score=100&age=18&pass=true</a>
		</li>
		<li>
			<a href="./mvc/case01/hello/sessionInfo">./mvc/case01/hello/sessionInfo</a>
		</li>
		
		<li>
			<a href="./mvc/case02/time/now">./mvc/case02/time/now</a>
		</li>
		<li>
			<a href="./mvc/case02/arraydata/">./mvc/case02/arraydata/</a>
		</li>
		<li>
			<a href="./mvc/case02/redirect/demo1">./mvc/case02/redirect/demo1</a>自動跳首頁
		</li>
		<li>
			<a href="./mvc/case02/redirect/demo2">./mvc/case02/redirect/demo2</a>自動跳time now
		</li>
		<li>
			<a href="./mvc/case02/redirect/demo3">./mvc/case02/redirect/demo3</a>自動跳yahoo
		</li>
		<li>
			<a href="./mvc/case02/redirect/demo4">./mvc/case02/redirect/demo4</a>自動跳代參數設定網頁
		</li>	
		<li>
			<a href="./mvc/case02/redirect/demo5">./mvc/case02/redirect/demo5</a>自動跳設定參數設定屬性
		</li>
		<li>
			<a href="./mvc/case02/redirect/saveOrder?name=iPhone&price=25000&qty=5">./mvc/case02/redirect/saveOrder?name=iPhone&price=25000&qty=5"</a>自動跳內網頁面,重新整理不會有反應
		</li>
		<li>
			<a href="./mvc/case02/lotto/">./mvc/case02/lotto/</a>
		</li>		
		<li>
			<a href="./mvc/case03/exam/">./mvc/case03/exam/</a>
		</li>
		<li>
			<a href="./mvc/case04/person/">./mvc/case04/person/</a>
		</li>
		<li>
			<a href="./mvc/case04/stock/">./mvc/case04/stock/</a>
		</li>
		<li>
			<a href="./mvc/lab/fund/">./mvc/lab/fund/</a>
		</li>
		<li>
			<a href="./mvc/lab/fundstock/">./mvc/lab/fundstock/</a>
		</li>
		<li>
			<a href="./html/fund.html">./html/fund.html</a>
		</li>
		
	</ol>
	
</body>
</html>